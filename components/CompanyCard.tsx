...
export default function CompanyCard({ company }: { company: Company }) {
  ...
  const rows: { label: string; value: string }[] =
    company.kind === "platform"
      ...
      : [
          { label: "買取方法", value: fmt(company.methods) },
          { label: "対象", value: fmt(company.target) },
          { label: "査定・出張料", value: `${fmt(company.hiyou.satei)} ／ ${fmt(company.hiyou.shucchou)}` },
          { label: "送料・返送料", value: `${fmt(company.hiyou.souryou)} ／ ${fmt(company.hiyou.hensou)}` },
          { label: "入金", value: fmt(company.nyukin) },
          { label: "故障・ジャンク", value: fmt(company.broken) },
        ];
  return (
