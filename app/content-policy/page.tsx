...
const sections: { t: string; body: string[] }[] = [
  {
    t: "1. 一次確認の方針",
    body: [
      "掲載する買取サービスの条件（買取方法・対応エリア・査定料・出張料・送料・梱包キット・返送料・査定日数・入金タイミング・キャンセル規定・本人確認・故障品やジャンクの受付・遺品や大量や法人への対応・クーリングオフの記載・運営会社・古物商許可番号）は、すべて各社の公式サイトおよび運営会社ページを当サイトが直接確認して記載します。第三者のまとめ記事・口コミサイトを情報源にしません。",
  ...
  },
  {
    t: "6. 金額・実績の表現",
    body: [
      "当サイトは、独自に検証していない具体的な買取金額・相場額・買取実績を掲載しません。カメラ・レンズの買取価格は状態と時期で変動するため、価格そのものは各社公式サイトの最新表示を確認する導線を置き、当サイトでは条件の比較に絞ります。金額に関する記載は、各社公式サイトの表示（送料の条件など）を出典つきで引用する場合に限ります。",
